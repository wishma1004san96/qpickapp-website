...
export function AppStoreBadge({
  ...
  const label =
    store === "ios" ? t("storeBadges.downloadOnAppStore") : t("storeBadges.getItOnGooglePlay");
  const aria =
    ...
    >
      ...
      <span className="flex flex-col items-start leading-tight">
        <span className="font-mono text-[0.6rem] uppercase tracking-wider text-foam/60">
          {t("storeBadges.comingSoon")}
        </span>
        <span className="text-sm font-medium">
          {store === "ios" ? t("storeBadges.appStore") : t("storeBadges.googlePlay")}
        </span>
      </span>
